...
const Hero = () => {
  ...
  const handleNavigateToChat = (message: string) => {
    ...
    // Navigate to create-new-trip with query param
    router.push(`/create-new-trip?message=${encodeURIComponent(message)}`);
  };

  const handleSendClick = (e: React.FormEvent) => {
    e.preventDefault();
    handleNavigateToChat(tripInput);
  };

  const handleSuggestionClick = (title: string) => {
    handleNavigateToChat(title);
  };
  ...
  return (
    <div className="flex flex-col justify-center items-center mt-10 px-4">
      ...
      {/* Input Box */}
      <form onSubmit={handleSendClick} className="relative w-full max-w-[600px] mt-6">
        <textarea
          name="trip"
          id="trip"
